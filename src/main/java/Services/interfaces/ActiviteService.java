...
/**
 * Couche d'accès aux données pour la table {@code activite} (CRUD).
 * <ul>
 *   <li><b>Create</b> : {@link #add(Activite)}, {@link #addAndReturnId(Activite)}</li>
 *   <li><b>Read</b>   : {@link #getAll()}, {@link #getById(int)}, {@link #searchByFilters(String, String, String, Double, Integer)}</li>
 *   <li><b>Update</b> : {@link #update(Activite)}</li>
...
public class ActiviteService implements IGenericService<Activite> {
    ...
    private static Activite mapRow(ResultSet rs) throws SQLException {
        Integer idPackObj = (Integer) rs.getObject("id_pack");
        int idPack = idPackObj != null ? idPackObj : 0;
    ...
    private static void setIdPack(PreparedStatement pst, int index, int idPack) throws SQLException {
        if (idPack <= 0) {
            pst.setNull(index, Types.INTEGER);
        } else {
            pst.setInt(index, idPack);
        }
    }

    private static void setNullableDouble(PreparedStatement pst, int index, Double v) throws SQLException {
        if (v == null) {
    ...
    public void add(Activite a) {
        String sql = "INSERT INTO activite (nom, type_activite, categorie_act, niveau_act, prix, statut, image_url, id_pack, latitude, longitude) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
            ...
            pst.setBigDecimal(5, BigDecimal.valueOf(a.getPrix()));
            pst.setString(6, normalizeStatut(a.getStatut()));
            pst.setString(7, a.getImageUrl() != null ? a.getImageUrl() : "");
            setIdPack(pst, 8, a.getIdPack());
            setNullableDouble(pst, 9, a.getLatitude());
            setNullableDouble(pst, 10, a.getLongitude());
            pst.executeUpdate();
    ...
    public void update(Activite a) {
        String sql = "UPDATE activite SET nom=?, type_activite=?, categorie_act=?, niveau_act=?, prix=?, statut=?, image_url=?, id_pack=?, latitude=?, longitude=? "
                + "WHERE id_activite=?";
        try (PreparedStatement pst = cnx.prepareStatement(sql)) {
            pst.setString(1, a.getNom());
            pst.setString(2, normalizeType(a.getTypeActivite()));
    ...
    public List<Activite> getAll() {
        ...
        String sql = "SELECT id_activite, nom, type_activite, categorie_act, niveau_act, prix, statut, image_url, id_pack, latitude, longitude FROM activite";
    ...
    public Activite getById(int id) {
        String sql = "SELECT id_activite, nom, type_activite, categorie_act, niveau_act, prix, statut, image_url, id_pack, latitude, longitude "
    ...
    public List<TopActivite> getTop3Activites() {
        ...
        String sql = "SELECT a.id_activite, a.nom, a.type_activite, a.categorie_act, a.niveau_act, "

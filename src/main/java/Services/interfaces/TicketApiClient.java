...
public class TicketApiClient {
    ...
    private String baseUrl() {
        // ✅ ensure server started, and get actual port
        LocalTicketServer.startOnce();
        return LocalTicketServer.getBaseUrl(); // ex: http://127.0.0.1:8090 or :8091...
    }
    ...
    public GenerateResp generate(int reservationId) throws Exception {
        ...
        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(baseUrl() + "/api/tickets/generate"))
    ...
    public VerifyResp verify(String token) throws Exception {
        ...
        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(baseUrl() + "/api/tickets/verify"))
                .header("Content-Type", "application/json")
                .POST(HttpRequest.BodyPublishers.ofString(json, StandardCharsets.UTF_8))
                .build();
